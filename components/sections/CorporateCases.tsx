...
export default function CorporateCases() {
  return (
    <section id="corporate-cases" className={`${brandDarkSurface} ${brandSection}`}>
      <div className={brandContainer}>
        <Reveal>
          <SectionIntro inverse eyebrow="Case Studies" title="企業が養蜂をする、という選択。">
            <p>
        ...
        <Reveal stagger className="mt-14 grid gap-4 md:grid-cols-3">
          {cases.map((item) => (
            <article key={item.company} className={`${brandCardWarm} flex flex-col p-7`}>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-bark">
                Since {item.since}
              </p>
              <h3 className="mt-4 text-xl font-semibold leading-tight text-ink">{item.company}</h3>
              <p className="mt-2 text-sm font-medium text-ink/60">{item.project}</p>
              <p className="mt-5 text-sm leading-7 text-ink/70">{item.body}</p>

              <dl className="mt-6 grid gap-3">
                <div className="border-t border-line pt-3">
                  <dt className="text-xs font-semibold uppercase tracking-[0.18em] text-bark">
                    Place
                  </dt>
                  <dd className="mt-1 text-sm leading-6 text-ink/70">{item.place}</dd>
                </div>
                {item.facts.map((fact) => (
                  <div key={fact} className="border-t border-line pt-3">
                    <dd className="text-sm leading-6 text-ink/70">{fact}</dd>
                  </div>
                ))}
              </dl>
              ...
              <a
                ...
                className="mt-auto inline-flex items-center gap-2 pt-7 text-xs font-semibold uppercase tracking-[0.18em] text-ink transition-colors hover:text-honey"
              >
                出典：{item.source}
